...
def main(argv):
    ...
    for train_indexes, test_index in train_test_indexes:
        ...
        similarities_train = []
        similarities_train.append(1.0)
        for i in range(0, len(X_train)-1):
            similarity = 1 - spatial.distance.cosine(embedding_action_matrix[X_train[i]], embedding_action_matrix[X_train[i+1]])
            similarities_train.append(similarity)
        best_threshold = 0
        best_G_MEAN = 0
        for threshold in [x * 0.1 for x in range(0, 10)]:
            cf_matrix_1 = get_conf_matrix_with_offset_strategy(similarities_train, y_train, timestamps_train, threshold, 1, lower=True)
        ...
        print("Calculated optimum threshold: " + str(best_threshold))
        print("Best G-MEAN value on TRAIN data (EXACT): " + str(best_G_MEAN))

        # calculate metrics in test data
        similarities_test = []
        similarities_test.append(1.0)
        for i in range(0, len(X_test)-1):
            similarity = 1 - spatial.distance.cosine(embedding_action_matrix[X_test[i]], embedding_action_matrix[X_test[i+1]])
            similarities_test.append(similarity)
        # TPR, TNR, FPR, G-MEAN for exact change point detection
        print("Total number of TRUE change points: "+ str(sum(y_test)))
        cf_matrix_1 = get_conf_matrix_with_offset_strategy(similarities_test, y_test, timestamps_test, best_threshold, 1, lower=True)
        TN, FP, FN, TP = cf_matrix_1.ravel()
        TPR = TP/(TP+FN)
        TNR = TN/(TN+FP)
        FPR = FP/(FP+TN)
        ...
        print("-----------------------------------------------------------------")
        print("TP 1s Exact: "+ str(TP))
        print("FP 1s Exact: "+ str(FP))
        print("TN 1s Exact: "+ str(TN))
        print("FN 1s Exact: "+ str(FN))
        ...
        cf_matrix_5 = get_conf_matrix_with_offset_strategy(similarities_test, y_test, timestamps_test, best_threshold, 5, lower=True)
        ...
        print("-----------------------------------------------------------------")
        print("TP 5s offset: "+ str(TP))
        print("FP 5s offset: "+ str(FP))
        print("TN 5s offset: "+ str(TN))
        print("FN 5s offset: "+ str(FN))
        ...
        cf_matrix_10 = get_conf_matrix_with_offset_strategy(similarities_test, y_test, timestamps_test, best_threshold, 10, lower=True)
        ...
        print("-----------------------------------------------------------------")
        print("TP 10s offset: "+ str(TP))
        print("FP 10s offset: "+ str(FP))
        print("TN 10s offset: "+ str(TN))
        print("FN 10s offset: "+ str(FN))
