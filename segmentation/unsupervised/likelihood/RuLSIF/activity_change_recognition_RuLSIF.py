...
def main(argv):
    ...
    for e in range(exe):
        ...
        scores_sum_norm = min_max_scaler.fit_transform(scores_sum.reshape(-1, 1)).flatten()
        scores_sum_norm = np.concatenate((np.zeros(n+k-1), scores_sum_norm))
        scores_sum_norm = np.concatenate((scores_sum_norm, np.zeros(n-1)))
        # save scores to pickle file
        scores_file = open(RESULTS_DIR + 'scores_file_exe_' + str(e) + '.pickle', 'wb')
        pickle.dump(scores_sum_norm, scores_file)
        scores_file.close()
        # plot in pieces
        if args.plot:
            points = 50
