...
def RuLSIF(x, y, alpha, sigma_range, lambda_range, kernel_num=100, verbose=False):
    ...
    def alpha_SEP_distance(x, y):
        ...
        n = x.shape[0]
        distance = abs((1/2) - ((1/n)*g_x.sum(axis=0)))
        return distance
...
# SEP score calculation alternative
def SEP(x_nu, x_de):
    x = np.c_[x_nu, x_de]

    dists = -2 * np.dot(x.T, x) + np.sum(x.T ** 2, axis=1) + np.sum(x.T ** 2, axis=1)[:, np.newaxis]
    dists = dists.reshape(x.shape[1] ** 2, 1)
    med = np.sqrt(0.5 * np.median(dists[dists > 0]))
    ...
    score_cv = np.zeros((np.size(sigma_list), np.size(lambda_list)))
    cv_index_nu = permutation(n_nu)
    cv_split_nu = np.floor(np.r_[0:n_nu] * fold / n_nu)

    for sigma_index in np.r_[0:np.size(sigma_list)]:
        sigma = sigma_list[sigma_index]
        K_nu = compute_kernel_Gaussian_2(x_nu,x_de, sigma).T
        score_tmp = np.zeros((fold, np.size(lambda_list)))

        for k in np.r_[0:fold]:
            mKtmp = np.mean(K_nu[:, cv_index_nu[cv_split_nu != k]], 1)

            for lambda_index in np.r_[0:np.size(lambda_list)]:
                lbd = lambda_list[lambda_index]
                thetat_cv = np.linalg.solve(lbd * np.eye(n_nu), mKtmp)
                thetah_cv = thetat_cv
                score_tmp[k, lambda_index] = abs(1 - np.mean(np.dot(mKtmp.T, thetah_cv).T))
    ...
    K_nu = compute_kernel_Gaussian_2(x_nu,x_de, sigma_chosen).T

    coe = lambda_chosen * np.eye(n_nu)

    var = np.mean(K_nu, 1)
    thetat = np.linalg.solve(coe, var)
    wh_x_nu = np.dot(K_nu.T, thetat).T
    SEP = abs((-1+ np.mean(wh_x_nu)))

    return SEP


# Compute kernel Gaussian function for SEP score calculation alternative
def compute_kernel_Gaussian_2(x1, x2, sigma):
    dist = np.sum(x1 ** 2, axis=0)[:, np.newaxis] + np.sum(x2 ** 2, axis=0) - 2 * np.dot(x1.T, x2)
