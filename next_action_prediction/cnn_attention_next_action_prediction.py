...
from models import CNN_EMBEDDING_LEVEL_ATTENTION

# M5 ARCHITECTURE (CNN WITH ATTENTION)
...
def main(argv):
    ...
    # init wandb
    run = wandb.init(project='har_futuraal', config=args, tags=['CNN_ATTENTION_NEXT_ACTION_PREDICTION'])
    config = wandb.config
    ...
    model = CNN_EMBEDDING_LEVEL_ATTENTION(args.number_of_actions, len(unique_actions), embedding_matrix, args.embedding_size, trainable_embeddings)
