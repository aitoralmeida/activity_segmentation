...
from models import GCN_CNN_MODEL
...
def main(argv):
    ...
    # init wandb
    run = wandb.init(project='har_futuraal', config=args, tags=['GCN_DIRECTED_CNN_NEXT_ACTION_PREDICTION'])
    config = wandb.config
    ...
    model = GCN_CNN_MODEL(args.number_of_actions, len(unique_actions), args.embedding_size, args.step_num)
    ...
    for key, value in action_index.items():
        G.add_node(value)
    X_actions_flattened = X_actions_train.flatten()
    for index in range(1, len(X_actions_flattened)):
        edge_1 = X_actions_flattened[index]
        edge_2 = X_actions_flattened[index-1]
