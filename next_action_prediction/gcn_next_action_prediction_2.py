...
def main(argv):
    ...
    # init wandb
    run = wandb.init(project='har_futuraal', config=args, tags=['GCN_2_NEXT_ACTION_PREDICTION'])
    config = wandb.config
    ...
    model = GCN_MODEL(args.number_of_actions, len(unique_actions), args.embedding_size, args.step_num)
    ...
    # prepare inputs nodes and adjacency matrixes
    nodes_train = []
    A_train = []
    nodes_test = []
    A_test = []
    for sequence in X_actions_train:
        ...
        for action in sequence:
            node_features.append(embedding_matrix[action])
        # create adjacency matrix from graph
        A = nx.adjacency_matrix(G).todense()
        # add adjacency matrix
        A_train.append(A)
        # add node_features
        nodes_train.append(node_features)
    ...
    model.fit([nodes_train, A_train], y_train, batch_size=32, epochs=1000, validation_data=([nodes_test, A_test], y_test), shuffle=False, callbacks=[checkpoint, WandbCallback()])
    ...
    predictions = model.predict([nodes_test, A_test], 32)
    correct = [0] * 5
    prediction_range = 5
