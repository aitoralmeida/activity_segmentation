...
from models import LSTM_EMBEDDING_LEVEL_ATTENTION

# A10 ARCHITECTURE (LSTM WITH ATTENTION)
...
def main(argv):
    ...
    # init wandb
    run = wandb.init(project='har_futuraal', config=args, tags=['LSTM_ATTENTION_NEXT_ACTION_PREDICTION'])
    config = wandb.config
    ...
    model = LSTM_EMBEDDING_LEVEL_ATTENTION(args.number_of_actions, len(unique_actions), embedding_matrix, args.embedding_size, trainable_embeddings)
